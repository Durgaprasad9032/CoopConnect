...
export const CustomerDashboard: React.FC = () => {
  ...
  const activeBooking = bookings.find((b) => 
    b.customerId === user?.uid && ['REQUESTED', 'ASSIGNED', 'ACCEPTED', 'ON_THE_WAY', 'STARTED'].includes(b.status)
  ) || bookings.find((b) => ['REQUESTED', 'ASSIGNED', 'ACCEPTED', 'ON_THE_WAY', 'STARTED'].includes(b.status));

  // Recent completed bookings
  const recentBookings = bookings.filter((b) => b.status === 'COMPLETED' || b.status === 'DISPUTED').slice(0, 3);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'REQUESTED':
        return <span className="bg-amber-100 text-amber-800 border border-amber-300 text-[11px] font-bold px-2.5 py-0.5 rounded-full">MATCHING...</span>;
      default:
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      {/* Active Booking Alert Card (if any) */}
      {activeBooking && (
        <div className="bg-white rounded-3xl p-6 border-2 border-teal-200 shadow-civic">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 text-xs">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50">
              <img
                src={activeBooking.workerPhoto || 'https://images.unsplash.com/photo-1540569014015-19a7be504e3a?auto=format&fit=crop&q=80&w=120'}
                alt={activeBooking.workerName || 'Worker'}
                className="w-10 h-10 rounded-full object-cover border border-slate-300"
              />
              <div>
                <span className="text-slate-500 block">Assigned Worker</span>
                <strong className="text-slate-900 text-sm">{activeBooking.workerName || 'Matching nearby worker...'}</strong>
                <span className="text-[11px] text-teal-700 block font-medium">{activeBooking.cooperativeName || 'Labor Guild'}</span>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Vetted trade technicians working part-time through local labor societies
            </p>
          </div>
          <Link
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DED1] shadow-civic">
        <div className="flex items-center justify-between mb-6 pb-3 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500">Completed jobs and past invoices</p>
          </div>
          <Link
              ...
              <div key={b.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-teal-50 text-[#0D6E66] flex items-center justify-center font-bold text-xs">
                    {b.serviceName.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Served by <strong>{b.workerName}</strong> ({b.cooperativeName}) on {b.date}
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-3">
                  <span className="font-bold text-sm text-slate-900">₹{b.actualAmount || b.estimatedAmount}</span>
                  <Link
